...
export default function BookPages() {
  ...
  console.error("Fetch Error:", err); // Log to console
  ...
  const addBook = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Backend expects { "bookName": "...", "author": "..." }
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Browse and manage collection</p>
        </div>
      </div>

      {/* 🟢 CONDITIONAL RENDER: Add Book Form (LIBRARIAN ONLY) */}
      {role === "LIBRARIAN" && (
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <form onSubmit={addBook} className="flex flex-col md:flex-row gap-3">
            ...
            <button
              disabled={submitting}
              className="flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-indigo-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed shadow-md shadow-indigo-200 cursor-pointer"
            >
              {submitting ? (
                "Adding..."
              ) : (
      ...
      {error ? (
        ...
        <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-500">
            {role === "LIBRARIAN" ? "Start by adding a new book above." : "Check back later."}
          </p>
        </div>
      ) : (
