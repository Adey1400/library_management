...
import { UserIcon, BookOpenIcon, ArrowLeftIcon } from "@heroicons/react/24/outline";
  ...
  if (!selectedRole) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Join LibraryHub</h1>
      ...
      <div className="max-w-xl w-full bg-white p-8 rounded-2xl shadow-xl border border-gray-100 relative">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900">
            {selectedRole === "STUDENT" ? "Student Registration" : "Librarian Access"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="firstname" onChange={handleChange} required className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">Last Name</label>
              <input name="lastname" onChange={handleChange} required className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none" />
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">Email Address</label>
            <input type="email" name="email" onChange={handleChange} required className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>
            ...
            <div className="pt-4 border-t border-gray-100 mt-4 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="department" placeholder="e.g. CS" onChange={handleChange} required className="w-full px-4 py-2 rounded-lg border border-indigo-200 bg-indigo-50 focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">Roll Number</label>
                  <input name="rollNo" placeholder="e.g. 2025001" onChange={handleChange} required className="w-full px-4 py-2 rounded-lg border border-indigo-200 bg-indigo-50 focus:ring-2 focus:ring-indigo-500 outline-none" />
                </div>
              </div>
            </div>
          )}

          <button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200 disabled:opacity-70 mt-6 cursor-pointer">
